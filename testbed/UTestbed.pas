...
var
  ...
  LErrors    : Integer;
begin
  TParseUtils.PrintLn(COLOR_YELLOW + 'NOTE: Run ' + COLOR_BOLD + 'reset.cmd' +
    COLOR_RESET  + COLOR_YELLOW +
    ' in the [bin\test] folder to restore unformatted test files.');
  TParseUtils.PrintLn();
  TParseUtils.Pause();
  ...
  try
    ...
    // --- FormatSource ---
    TParseUtils.PrintLn(COLOR_BOLD + COLOR_CYAN + '=== FormatSource ===');
    LSource    := 'procedure Foo; begin writeln(''hello''); end;';
    LFormatted := LPasFmt.FormatSource(LSource, LOptions);
    TParseUtils.PrintLn(COLOR_WHITE + LFormatted + COLOR_RESET);
    TParseUtils.Pause('Press ENTER for next test..');

    // --- FormatFile ---
    TParseUtils.PrintLn(COLOR_BOLD + COLOR_CYAN + '=== FormatFile ===');
    LResult := LPasFmt.FormatFile('test\src\test_unit_complex.pas', LOptions);
    if LResult.Success then
      TParseUtils.PrintLn(COLOR_GREEN + '[OK]  ' +
        LResult.FilePath + ' — ' + IfThen(LResult.Changed, 'Changed', 'No change'))
    else
      TParseUtils.PrintLn(COLOR_RED + '[ERR] ' +
        LResult.FilePath + ' — ' + LResult.ErrorMsg);
    TParseUtils.Pause('Press ENTER for next test..');

    // --- FormatFolder ---
    TParseUtils.PrintLn(COLOR_BOLD + COLOR_CYAN + '=== FormatFolder ===');
    LChanged := 0;
    LErrors  := 0;
    LResults := LPasFmt.FormatFolder('test\src', LOptions);
    ...
    TParseUtils.PrintLn(COLOR_YELLOW + '%d files processed, %d changed, %d errors',
