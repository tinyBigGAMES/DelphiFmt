...
type
  ...
  private
    ...
    // ---- attribute helpers ----
    function AttrStr(const ANode: TParseASTNode; const AKey: string): string;
    function AttrBool(const ANode: TParseASTNode; const AKey: string): Boolean;
    function AttrInt(const ANode: TParseASTNode; const AKey: string): Integer;

    // ---- safe child accessor ----
    function Ch(const ANode: TParseASTNode; const AIdx: Integer): TParseASTNode;
    ...
    function EmitExpr(const ANode: TParseASTNode): string;

    // ---- type-reference emitter (returns string, never writes FOutput) ----
    function EmitTypeRef(const ANode: TParseASTNode): string;

    // ---- param list emitter (returns string) ----
    function EmitParamList(const ANode: TParseASTNode): string;

    // ---- statement visitors (write to FOutput) ----
    procedure VisitStmt(const ANode: TParseASTNode);

    procedure VisitUnit(const ANode: TParseASTNode);
    procedure VisitProgram(const ANode: TParseASTNode);
    procedure VisitLibrary(const ANode: TParseASTNode);
    procedure VisitSection(const ANode: TParseASTNode;
      const AInIntf: Boolean);
    procedure VisitUsesClause(const ANode: TParseASTNode);

    procedure VisitConstBlock(const ANode: TParseASTNode);
    procedure VisitVarBlock(const ANode: TParseASTNode);
    procedure VisitTypeBlock(const ANode: TParseASTNode);
    procedure VisitTypeDecl(const ANode: TParseASTNode);
    procedure VisitClassBody(const ATypeNode: TParseASTNode;
      const AKeyword: string; const ADeclName: string);

    procedure VisitVisibility(const ANode: TParseASTNode);
    procedure VisitFieldDecl(const ANode: TParseASTNode);
    procedure VisitPropertyDecl(const ANode: TParseASTNode);

    procedure VisitRoutineDecl(const ANode: TParseASTNode);
    procedure VisitRoutineSigLine(const ANode: TParseASTNode);

    procedure VisitBeginBlock(const ANode: TParseASTNode;
      const ATrailSemi: Boolean);
    procedure VisitBody(const ANode: TParseASTNode);

    procedure VisitIfStmt(const ANode: TParseASTNode);
    procedure VisitWhileStmt(const ANode: TParseASTNode);
    procedure VisitForStmt(const ANode: TParseASTNode);
    procedure VisitRepeatStmt(const ANode: TParseASTNode);
    procedure VisitCaseStmt(const ANode: TParseASTNode);
    procedure VisitWithStmt(const ANode: TParseASTNode);
    procedure VisitTryStmt(const ANode: TParseASTNode);
    procedure VisitOnStmt(const ANode: TParseASTNode);
    procedure VisitRaiseStmt(const ANode: TParseASTNode);
    procedure VisitDirective(const ANode: TParseASTNode);
    procedure VisitExprStmt(const ANode: TParseASTNode);

  public
    constructor Create();
    destructor  Destroy(); override;

    function FormatTree(const ARoot: TParseASTNode;
...
// =============================================================================
function TDelphiFmtEmitter.FormatTree(const ARoot: TParseASTNode;
  const AOptions: TDelphiFmtOptions): string;
begin
  FOutput.Clear();
  FOptions     := AOptions;
...
function TDelphiFmtEmitter.AttrStr(const ANode: TParseASTNode;
...
function TDelphiFmtEmitter.AttrBool(const ANode: TParseASTNode;
...
function TDelphiFmtEmitter.AttrInt(const ANode: TParseASTNode;
...
function TDelphiFmtEmitter.Ch(const ANode: TParseASTNode;
  const AIdx: Integer): TParseASTNode;
begin
  if (ANode = nil) or (AIdx < 0) or (AIdx >= ANode.ChildCount()) then
    Result := nil
  else
    Result := TParseASTNode(ANode.GetChild(AIdx));
end;
...
function TDelphiFmtEmitter.EmitExpr(const ANode: TParseASTNode): string;
var
  LKind  : string;
...
function TDelphiFmtEmitter.EmitTypeRef(const ANode: TParseASTNode): string;
var
  LKind  : string;
  LParts : TStringBuilder;
  LI     : Integer;
...
function TDelphiFmtEmitter.EmitParamList(const ANode: TParseASTNode): string;
var
  LParts    : TStringBuilder;
  LI        : Integer;
  LGroup    : TParseASTNode;
  LMod      : string;
  LNames    : string;
...
procedure TDelphiFmtEmitter.VisitStmt(const ANode: TParseASTNode);
var
  LKind : string;
  LI    : Integer;
begin
  ...
  begin
    // Labels sit one indent level lower per Castalia default
  ...
  begin
    // ASM blocks: emit keyword and a placeholder end; we have no token list
    Append(KW(ANode.GetToken().Text));
    NL();
    Append(KW(AttrStr(ANode, 'kw.end')) + ';');
    NL();
  end
  else if (LKind = 'comment.line') or (LKind = 'comment.block') then
  begin
    Append(ANode.GetToken().Text);
    NL();
  end
...
procedure TDelphiFmtEmitter.VisitUnit(const ANode: TParseASTNode);
var
  LUnitName : string;
  LI        : Integer;
  LChild    : TParseASTNode;
...
procedure TDelphiFmtEmitter.VisitProgram(const ANode: TParseASTNode);
var
  LName  : string;
  LI     : Integer;
  LJ     : Integer;
...
procedure TDelphiFmtEmitter.VisitLibrary(const ANode: TParseASTNode);
var
  LName  : string;
  LI     : Integer;
  LJ     : Integer;
...
procedure TDelphiFmtEmitter.VisitSection(const ANode: TParseASTNode;
  const AInIntf: Boolean);
var
  LI        : Integer;
  LChild    : TParseASTNode;
...
procedure TDelphiFmtEmitter.VisitUsesClause(const ANode: TParseASTNode);
var
  LI    : Integer;
  LItem : TParseASTNode;
  LName : string;
...
procedure TDelphiFmtEmitter.VisitConstBlock(const ANode: TParseASTNode);
var
  LI        : Integer;
  LDecl     : TParseASTNode;
  LName     : string;
...
procedure TDelphiFmtEmitter.VisitVarBlock(const ANode: TParseASTNode);
var
  LI       : Integer;
  LDecl    : TParseASTNode;
  LNames   : string;
...
procedure TDelphiFmtEmitter.VisitTypeBlock(const ANode: TParseASTNode);
var
  LI           : Integer;
  LDecl        : TParseASTNode;
  LTypeRef     : TParseASTNode;
  LCurComplex  : Boolean;
...
procedure TDelphiFmtEmitter.VisitTypeDecl(const ANode: TParseASTNode);
var
  LName    : string;
  LTypeRef : TParseASTNode;
  LKind    : string;
...
procedure TDelphiFmtEmitter.VisitClassBody(const ATypeNode: TParseASTNode;
  const AKeyword: string; const ADeclName: string);
var
  LI         : Integer;
  LChild     : TParseASTNode;
...
procedure TDelphiFmtEmitter.VisitVisibility(const ANode: TParseASTNode);
var
  LVis    : string;
  LStrict : Boolean;
begin
...
procedure TDelphiFmtEmitter.VisitFieldDecl(const ANode: TParseASTNode);
var
  LNames : string;
  LArr   : TArray<string>;
  LJ     : Integer;
...
procedure TDelphiFmtEmitter.VisitPropertyDecl(const ANode: TParseASTNode);
var
  LPropName  : string;
...
procedure TDelphiFmtEmitter.VisitRoutineDecl(const ANode: TParseASTNode);
var
  LI         : Integer;
  LChild     : TParseASTNode;
  LKind      : string;
...
procedure TDelphiFmtEmitter.VisitRoutineSigLine(const ANode: TParseASTNode);
var
  LKind     : string;
  LDeclKind : string;
  LName     : string;
...
procedure TDelphiFmtEmitter.VisitBeginBlock(const ANode: TParseASTNode;
  const ATrailSemi: Boolean);
var
  LI    : Integer;
begin
...
procedure TDelphiFmtEmitter.VisitBody(const ANode: TParseASTNode);
begin
  if ANode = nil then
    Exit;
...
procedure TDelphiFmtEmitter.VisitIfStmt(const ANode: TParseASTNode);
var
  LCond     : TParseASTNode;
  LThen     : TParseASTNode;
  LElse     : TParseASTNode;
...
procedure TDelphiFmtEmitter.VisitWhileStmt(const ANode: TParseASTNode);
begin
...
procedure TDelphiFmtEmitter.VisitForStmt(const ANode: TParseASTNode);
var
  LVar   : string;
  LKind  : string;
  LDir   : string;
...
procedure TDelphiFmtEmitter.VisitRepeatStmt(const ANode: TParseASTNode);
var
  LI     : Integer;
  LUntil : TParseASTNode;
begin
  Append(KW(ANode.GetToken().Text));
  NL();
  IndentIn();
...
procedure TDelphiFmtEmitter.VisitCaseStmt(const ANode: TParseASTNode);
var
  LI        : Integer;
  LChild    : TParseASTNode;
  LKind     : string;
...
procedure TDelphiFmtEmitter.VisitWithStmt(const ANode: TParseASTNode);
var
  LLine : TStringBuilder;
  LI    : Integer;
  LBody : TParseASTNode;
...
procedure TDelphiFmtEmitter.VisitTryStmt(const ANode: TParseASTNode);
var
  LI     : Integer;
  LChild : TParseASTNode;
  LKind  : string;
...
procedure TDelphiFmtEmitter.VisitOnStmt(const ANode: TParseASTNode);
var
  LVarName  : string;
  LTypeName : string;
  LTypeRef  : TParseASTNode;
...
procedure TDelphiFmtEmitter.VisitRaiseStmt(const ANode: TParseASTNode);
begin
  if ANode.ChildCount() > 0 then
    Append(KW(ANode.GetToken().Text) + ' ' + EmitExpr(Ch(ANode, 0)) + ';')
  else
...
procedure TDelphiFmtEmitter.VisitDirective(const ANode: TParseASTNode);
var
  LRaw : string;
begin
...
procedure TDelphiFmtEmitter.VisitExprStmt(const ANode: TParseASTNode);
begin
