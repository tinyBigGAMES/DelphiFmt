...
procedure RegisterNumbers(const AParse: TParse);
begin
  AParse.Config()
    .SetHexPrefix('$', PARSE_KIND_INTEGER);
end;
...
procedure RegisterIdentifiers(const AParse: TParse);
begin
  AParse.Config()
...
procedure ConfigLexer(const AParse: TParse);
begin
  RegisterIdentifiers(AParse);
  RegisterKeywords(AParse);
  RegisterOperators(AParse);
  RegisterNumbers(AParse);
