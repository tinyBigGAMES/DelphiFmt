...
function  ParseTypeReference(const AParser: TParseParserBase): TParseASTNode; forward;
function  ParseParamList(const AParser: TParseParserBase): TParseASTNode; forward;
procedure ParseRoutineDirectives(const AParser: TParseParserBase;
  const ANode: TParseASTNode); forward;
function  ParseUsesClause(const AParser: TParseParserBase; const AUsesKW: string): TParseASTNode; forward;
function  ParseRoutineSignature(const AParser: TParseParserBase;
  const AIsFunction: Boolean; const ANodeKind: string): TParseASTNode; forward;
procedure ParseRoutineBody(const AParser: TParseParserBase;
  const ANode: TParseASTNode); forward;
function  ParseFieldDecl(const AParser: TParseParserBase): TParseASTNode; forward;
function  ParsePropertyDecl(const AParser: TParseParserBase): TParseASTNode; forward;
function  ParseParamList_Bracket(const AParser: TParseParserBase): TParseASTNode; forward;
function  ParseRecordCase(const AParser: TParseParserBase): TParseASTNode; forward;
...
function IsDeclarationStart(const AParser: TParseParserBase): Boolean;
begin
  Result :=
...
function IsRoutineDirective(const AParser: TParseParserBase): Boolean;
begin
  Result :=
    // Calling conventions
...
function ParseTypeReference(const AParser: TParseParserBase): TParseASTNode;
var
  LNode:     TParseASTNode;
  LParamList: TParseASTNode;
  LName:     string;
  LSrictKW:  string;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TParseASTNode(LNode.GetChild(LNode.ChildCount() - 1)).SetAttr(
...
function ParseParamList(const AParser: TParseParserBase): TParseASTNode;
var
  LListNode:  TParseASTNode;
  LGroupNode: TParseASTNode;
  LModifier:  string;
  LNames:     string;
  LNameCount: Integer;
begin
  ...
  begin
    ...
    if AParser.Match('op.eq') then
      LGroupNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));
...
procedure ParseRoutineDirectives(const AParser: TParseParserBase;
  const ANode: TParseASTNode);
var
  LDirNode: TParseASTNode;
begin
  while IsRoutineDirective(AParser) do
  begin
    ...
    begin
      ...
      if AParser.Check(PARSE_KIND_STRING) or
         AParser.Check(PARSE_KIND_IDENTIFIER) then
      begin
...
function ParseUsesClause(const AParser: TParseParserBase; const AUsesKW: string): TParseASTNode;
var
  LNode:     TParseASTNode;
...
function ParseRoutineSignature(const AParser: TParseParserBase;
  const AIsFunction: Boolean; const ANodeKind: string): TParseASTNode;
var
  LNode:      TParseASTNode;
  LName:      string;
begin
...
begin
  ...
  while AParser.Check('keyword.var') or
    ...
    ANode.AddChild(TParseASTNode(AParser.ParseStatement()));
...
function ParseFieldDecl(const AParser: TParseParserBase): TParseASTNode;
var
  LNode:  TParseASTNode;
  LNames: string;
  LCount: Integer;
...
function ParsePropertyDecl(const AParser: TParseParserBase): TParseASTNode;
var
  LNode:     TParseASTNode;
  LPropName: string;
begin
...
function ParseParamList_Bracket(const AParser: TParseParserBase): TParseASTNode;
var
  LListNode:  TParseASTNode;
  LGroupNode: TParseASTNode;
  LModifier:  string;
...
function ParseRecordCase(const AParser: TParseParserBase): TParseASTNode;
var
  LNode:    TParseASTNode;
...
begin
  ...
  begin
    ...
    // Label(s)
    LVarNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));
    while AParser.Match('delimiter.comma') do
      LVarNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));
    AParser.Expect('delimiter.colon');
    // Fields in parens
...
procedure RegisterUnaryOps(const AParse: TParse);
begin
...
procedure RegisterInheritedExpr(const AParse: TParse);
begin
  ...
  AParse.Config().RegisterInfixLeft('op.lt', 10, 'expr.binary',
    ...
    begin
      ...
      begin
        ...
        if (LTok.Kind = PARSE_KIND_IDENTIFIER) or
      ...
      begin
        ...
        LGenNode.AddChild(TParseASTNode(ALeft));
        AParser.Consume();  // consume '<'
...
procedure RegisterTypeTestOps(const AParse: TParse);
begin
...
begin
  AParse.Config().RegisterInfixLeft('delimiter.lparen', 40, 'expr.call',
    ...
    var
      LNode:    TParseASTNode;
      LArgNode: TParseASTNode;
      LFmtNode: TParseASTNode;
    begin
      LNode := AParser.CreateNode('expr.call', ALeft.GetToken());
      AParser.Consume();  // consume '('
      LNode.AddChild(TParseASTNode(ALeft));  // callee as first child

      if not AParser.Check('delimiter.rparen') then
      begin
        LArgNode := TParseASTNode(AParser.ParseExpression(0));
        // Check for format specifier: expr:width[:decimals]
        if AParser.Check('delimiter.colon') and
           (AParser.PeekToken(1).Kind = PARSE_KIND_INTEGER) then
        begin
...
procedure RegisterRangeOp(const AParse: TParse);
begin
...
begin
  AParse.Config().RegisterStatement('keyword.begin', 'stmt.begin_block',
    ...
    var
      ...
      LLabelNode: TParseASTNode;
      LLabelTok:  TParseToken;
    begin
      LNode := AParser.CreateNode();
      ...
      begin
        ...
        if AParser.Check(PARSE_KIND_IDENTIFIER) and
  ...
  AParse.Config().RegisterStatement('keyword.for', 'stmt.for',
    ...
    begin
      ...
      begin
        ...
        AParser.Consume();  // consume 'in'
        LNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));  // collection
      end
      else
      begin
...
procedure RegisterWithStmt(const AParse: TParse);
begin
...
procedure RegisterOnStmt(const AParse: TParse);
begin
  ...
  AParse.Config().RegisterStatement('keyword.raise', 'stmt.raise',
    ...
    begin
      ...
      if not AParser.Check('delimiter.semicolon') and
         not AParser.Check('keyword.end') and
         not AParser.Check(PARSE_KIND_EOF) then
      begin
        LNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));
...
procedure RegisterGotoStmt(const AParse: TParse);
begin
...
procedure RegisterLabelDecl(const AParse: TParse);
begin
...
procedure RegisterAsmBlock(const AParse: TParse);
begin
  ...
  AParse.Config().RegisterStatement('keyword.var', 'stmt.var_block',
    ...
    begin
      ...
      begin
        ...
        if AParser.Match('op.eq') then
          LDeclNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));
...
procedure RegisterThreadvarBlock(const AParse: TParse);
begin
...
procedure RegisterResourcestringBlock(const AParse: TParse);
begin
  AParse.Config().RegisterStatement('keyword.resourcestring',
    ...
    begin
      ...
      begin
        ...
        AParser.Consume();
        AParser.Expect('op.eq');
        LRSNode.AddChild(TParseASTNode(AParser.ParseExpression(0)));
        AParser.Expect('delimiter.semicolon');
...
procedure RegisterConstructorDecl(const AParse: TParse);
begin
...
procedure RegisterDestructorDecl(const AParse: TParse);
begin
...
procedure RegisterDirectiveStmt(const AParse: TParse);
begin
  ...
  AParse.Config().RegisterStatement('keyword.program', 'stmt.program',
    ...
    begin
      ...
      begin
        ...
        while not AParser.Check('keyword.end') and
              not AParser.Check(PARSE_KIND_EOF) do
          LBodyNode.AddChild(TParseASTNode(AParser.ParseStatement()));
...
begin
  AParse.Config().RegisterStatement('keyword.unit', 'stmt.unit',
    ...
    var
      ...
      LInitNode: TParseASTNode;
      LFinNode:  TParseASTNode;
      LUnitName: string;
      LUsesKW:   string;
    begin
      LNode := AParser.CreateNode();
      ...
      begin
        ...
        while not AParser.Check('keyword.finalization') and
              ...
              not AParser.Check(PARSE_KIND_EOF) do
        begin
          LInitNode.AddChild(TParseASTNode(AParser.ParseStatement()));
        end;
      ...
      begin
        ...
        while not AParser.Check('keyword.end') and
              not AParser.Check(PARSE_KIND_EOF) do
        begin
          LFinNode.AddChild(TParseASTNode(AParser.ParseStatement()));
        end;
...
procedure RegisterExportsClause(const AParse: TParse);
begin
